...
c***********************************************************************
      ...
      real*8 dsol,dmix,db,fac_sal,vq,porX,dbdepth,attdb
	integer isolid

	 porX = 0.75d0   ! I need to figure out how to pass these from 
	 dbdepth = 10.0 ! Maple.  VK 10/27/09.
         attdb = 0.0d0    !                        
...
c        vs(j) = vs(j) + w
         ...
         vd(j)=vd(j)+(w0*(1-por0)*porX)/((1-porX)*por(j))+vq
         vs(j)=vs(j)+(w0*(1-por0))/(1-por(j))

c	  vd(j)=vd(j)-(w0*(1-por0)*porX)/
c     +  ((1-porX)*((porX-por0)*exp(-attpor*x(j))-porX))
c        vs(j)=vs(j)+w0*(1-por0)/(1-porX+(porX-por0)*exp(-attpor*x(j)))
...
c           db = max((db0*0.5d0*erfcc((x(j)-dbdepth)/attdb)),1.d-20) ! F
c          db = db0/2.0d0*erfcc((x(j)-10.0d0)/4.0d0)
c         db=0.1d0
          db=db0 ! PP - constant due to MultiG approach

           if(x(j).gt.xbiot)  then
           db=0.d0
           endif 
...
c		original dsol = dsol/(1.0d0-dlog(por(j)*por(j)))			! in situ Dmol
...
c	        pause 'hit any key to continue'
	      end if
	    else
